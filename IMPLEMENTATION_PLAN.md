...
## 15. 2026-09-09 全局决策与局部上下文关闭增补

新增高风险任务 `T09A`：把历史上下文拆分为版本化全局决策和每个具体 Agent 独占的局部上下文偏序图；通过本地闭包、关闭胶囊和结构化回访减少重复提示词，同时支持 Assist 阻塞验收与 Devolve 延迟人工核验。完整任务卡：`docs/tasks/T09A_CONTEXT_NODE_CLOSURE_AND_RECALL_TASK_CARD.md`。

冻结边界：

1. 任务完成事件与上下文关闭事件独立；Agent 不能自行签字或直接写关闭标记。
2. 关键设计理由、长期约束和接口契约在局部关闭前提升为带来源、revision、哈希、范围和失效条件的全局决策记录；全局决策库与模型可见全局上下文分离。公开设置 `maximumGlobalContextTokenCount` 默认4096个估算token，认证用户可调高、调低或设为0；运行时还要为系统、任务和输出保留空间，并只选择具有显式高相关关系的最小内容。
3. 无关内容和被预算淘汰的详情写入按具体Agent/任务隔离的 `GLOBAL_CONTEXT_DEFERRED_FRAGMENT_V1` 文件，后续相关任务按来源、关系和revision受控读取；不得进入稳定提示词前缀或其他Agent记忆。
4. 关闭只从普通提示词逻辑排除历史，不删除存档；物理清理仍执行自动备份和原有破坏性门禁。
5. Assist 默认 `block-until-verified`，认证用户可切换为 `continue-with-deferred-review`；Devolve 默认相反。延迟模式必须先创建层级 1 或以下的人工核验任务，并显式标记未获用户验收。
6. 回访依次返回索引、关闭胶囊、选定证据和有界完整片段；敏感禁读、个体记忆隔离、反自指、活锁和读取预算均不可绕过。
7. 缓存分为全局决策块、活跃前沿、关闭胶囊和回访结果，重点统计可复用 token、全局预算淘汰、延后片段命中与回访质量，而不是只追求 hit 数量。
8. 当前仓库的 T12A/T12 已先于本增补实现；T09A 必须复用其恢复原语，并在完成后重新执行 T12A/T12 的恢复、安全和 tarball 回归，不得假定既有验收自动覆盖新增状态。
...
T09A 每轮只执行一个检查点，每个检查点预计不超过3小时；不能与 T07D、GUI、T12 综合加固或最终打包合批。生产变量和函数必须使用完整可读名称，计数、字节、token 和时间变量必须明确单位。
