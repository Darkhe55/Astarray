...
| T09A | 全局决策提升、局部上下文节点关闭与分级回访 | in_progress | post-T12增补 | T09A-01~04 完成（契约 schema、局部上下文图存储、全局决策提升/选择/延后片段、人工验收策略与延迟核验/否决返修；9+10+12 新测试与相关回归全绿；typecheck/lint 绿）；受沙箱限制全量 `npm run check`（build/spawn 类）待审批补跑；T09A-05~08 待续 |
