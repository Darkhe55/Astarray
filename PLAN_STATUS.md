...
| T09A | 全局决策提升、局部上下文节点关闭与分级回访 | in_progress | post-T12增补 | T09A-01~05 完成（契约 schema、图存储、全局决策提升/选择/延后片段、人工验收与延迟核验、关闭胶囊/活跃前沿/分级回访、分层缓存与生命周期指标；7+9+9+10+12 新测试与 59 例 T09A 回归全绿；typecheck/lint 绿）；受沙箱限制全量 `npm run check`（build/spawn 类）待审批补跑；T09A-07/08 待续 |
