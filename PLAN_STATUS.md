...
| T09A | 全局决策提升、局部上下文节点关闭与分级回访 | in_progress | post-T12增补 | T09A-01/02/03 完成（契约 schema、局部上下文图存储、全局决策库提升与相关选择/预算/延后片段；10+12 新测试与 70 相关套件回归全绿；typecheck/lint 绿）；受沙箱限制全量 `npm run check`（build/spawn 类）待审批补跑；T09A-04~08 待续 |
