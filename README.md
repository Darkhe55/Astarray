...
TUI Agent 编排工具：单一主 Agent + Ponder / Assist / Devolve 三模式。主 Agent 派发任务后立即恢复接收用户输入；三级 Agent（主 → 次级调度/集成 → 三级执行）通过**独立反馈进程**解耦通信。任务按 DAG 调度、按最小工具集授权执行；涉及 Git 写入时，由次级 Agent 统一分配隔离分支/worktree、审查三级 Agent 提交并执行受控合并。
