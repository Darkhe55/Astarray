...
- `builtin:assist`：使用上表的独立默认矩阵。安装按**实际作用范围**裁决（ADR-0039/0043）：项目内受控安装即使配置为 `allow`，仍须满足 ADR-0019 的已有资源询问、独立开关与精确参数绑定，可由有权上级按设置批准；跨项目根、项目外缓存/环境写入、全局安装与外部软件控制仍需逐次用户授权。
