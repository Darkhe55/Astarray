...
## 补充（GUIDE 增量 02b：新任务插入任务偏序集）

39. **插入目标必填**：new-task 必须携带 `insertionTarget`（ownerAgentInstanceId、sequenceId、
    观察到的 sequence revision、可选前驱/后继锚点）；缺失 → 澄清，不插入、不登记变更意图。
40. **复用既有偏序集**：插入走 `TaskSequenceManageController.insertTask`（ADR-0013 的偏序/优先级/审计），
    **不新建第二套调度**；来源为用户（认证用户指导），优先级层级按既有策略解析（用户默认 0）。
41. **插入先于登记**：先插入再登记变更意图；插入失败（并发 revision、重复标识、未知锚点等）
    → 返回 `task-insertion-failed` 且**不登记变更意图、不改任务历史**。
42. **锚点与不可变**：前驱/后继锚点由偏序集校验（环/未知锚点拒绝）；序列 revision 单调前进。
43. **未接入（诚实声明）**：长任务中途追加/撤销的**运行中端到端**（真实长任务安全点消费 + 观测）
    仍依赖运行时执行；本轮以公共入口 + 真实偏序集插入验证，未做真实 Provider 长任务演练。
