...
| 次级调度/集成 Agent（Assist） | 切割任务成 DAG、分配最小工具集、调度 Worker；负责 Git 分支与 worktree 分流、差异审查、测试验证和受控合并；处理反馈，无法裁决时转回用户 | harness 内置提示词：全量工具 + 用法 |
| 三级执行 Agent（Worker） | 仅执行分配任务；代码修改只能提交到自己的隔离分支，不能自行合并、变基、推送或删除分支；异常/模糊/完成均走反馈工具上报，然后待机 | 任务所需工具子集 |

Devolve 模式下主 Agent 可直接决定调度策略；只要任务使用多个 Git 写入 Worker，仍必须指定一个次级集成 Agent 负责分流、审查和技术合并，复用同一 task store、反馈进程、Worker 与工具策略边界。
...
- 控制面 `taskSequenceStatus` 提供只读序列、任务、ready set、顺序解释和任务包查询。用户在任意模式都可从 TUI/CLI 查看，不会打断 busy Agent；Ponder 模型仍无工具权限。
