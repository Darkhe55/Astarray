...
- GUI-01-R-04b 权限组切换校验与缺陷发现（2026-09-19）：同一脚本扩到 27 项——内置组切换成功且重读一致、非法内置组 400、无 CSRF 403、校验后恢复原状；**发现缺陷**：`POST /commands/switch-permission-profile` 接受不存在的自定义权限组并返回 200（持久化悬空引用，解析侧 fail-closed 但报错码误用 `task-sequence-not-found`），**已于 2026-09-21 修复**：新增稳定错误码 `permission-profile-not-found`、控制器持久化前校验自定义组存在、GUI 映射 404；先红（4 失败）后绿（全量 228 文件/1857 用例、覆盖率 93.07/85.24/93.09/93.10、安全模块 22/22、安装产物 GUI 契约 27/27）。同轮收口：`custom-permission-profile-controller.ts` 的 3 处错误码误用一并改为 `permission-profile-not-found`（目标 3 文件 38 用例、全量 228 文件/1857 用例、覆盖率 93.07/85.22/93.09/93.09、安全模块 22/22）。详见 docs/reports/GUI01_R_04B_DANGLING_PROFILE_SELECTION_FINDING_2026-09-19.md §6。
